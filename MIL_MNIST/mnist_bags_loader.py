...
class MnistBags(data_utils.Dataset):
    def __init__(self, target_number=9, mean_bag_length=10, var_bag_length=1, num_bag=1000, seed=7, train=True):
        ...
        self.r = np.random.RandomState(seed)    # 伪随机数种子
    ...
    def _form_bags(self):
        if self.train:
            ...
            # batch_size=self.num_in_train，一次性取完所有图片
            ...
            while valid_bags_counter < self.num_bag:
                ...
                if bag_length < 1:
                    bag_length = 1
                # 0到num_in_train之间，随机抽bag_length个数
                indices = torch.LongTensor(self.r.randint(0, self.num_in_train, bag_length))
                # 记录抽到的数字的标签，是一个数而不是独热编码
                labels_in_bag = labels[indices]

                if (self.target_number in labels_in_bag) and (label_of_last_bag == 0):
                    labels_in_bag = labels_in_bag >= self.target_number  # 数字‘9’的标签为True，其他为False
                    # 每个包的实例标签都放在labels_list内
                ...
                elif label_of_last_bag == 1:    # 生成负包
                    index_list = []
                    bag_length_counter = 0  # 单个包长度
...
if __name__ == "__main__":
    ...
    len_bag_list = []
    mnist_bags_train = 0
    #  一次取一个包，自定义的__getitem__会使标签形式变成（bag-label, instances-labels）
    for batch_idx, data in enumerate(train_loader):  # data包含样本和标签，data的结构是（bag_list, label_list）
        plot_data = data[0].squeeze(0)  # 得到包样本
        len_bag_list.append(int(plot_data.size()[0]))  # 包的长度作为1个元素存入len_bag_list中
        # 将这个包的图片画出来
        ...
        # data[1]是标签（bag-label, instances-labels）
        # data[1]格式:[tensor([False]), tensor([[False, False, False, False, False, False, False, False, False]])]
        if data[1][0][0] == 1:
            mnist_bags_train += 1  # 含有‘9’的包
